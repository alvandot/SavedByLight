...
setup_build_env() {
    ...
    # Clean or create SavedByLight directory
    if [ -d "$LIGHT_DIR" ]; then
        rm -rf "$LIGHT_DIR"/*
    else
        mkdir -p "$LIGHT_DIR"
...
setup_localversion() {
    ...
    local localversion="-SavedByLight-${MODEL}-${BUILD_KERNEL_VERSION}-A${PLATFORM_VERSION}FS"
...
compile_kernel() {
    ...
    if ! make ARCH=arm64 -j$(nproc --all) O=${OUT_DIR} exynos9820-${DEVICE}_defconfig SavedByLight.config ksu.config; then
...
create_odin_package() {
    ...
    if ! tar -cvf "SavedByLight_${MODEL}_v.${BUILD_KERNEL_VERSION}_Odin_SUSFS.tar" boot.img dt.img dtbo.img; then
...
create_twrp_package() {
    ...
    if ! zip -r "SavedByLight_${MODEL}_v.${BUILD_KERNEL_VERSION}_TWRP_SUSFS.zip" META-INF boot.img dt.img dtbo.img; then
...
main() {
    ...
    echo "   - SavedByLight_${MODEL}_v.${BUILD_KERNEL_VERSION}_Odin_SUSFS.tar"
    echo "   - SavedByLight_${MODEL}_v.${BUILD_KERNEL_VERSION}_TWRP_SUSFS.zip"
    echo ""
